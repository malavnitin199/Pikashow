...
function Movies() {
  const [catagory1, setCatagory1] = useState("popular");
  const options1 = ["popular", "top_rated", "upcoming", "now_playing"];
  ...
  const getData = async () => {
    try {
      const { data } = await axios.get(
        `/movie/${catagory1}?language=en-US&page=${page}`
      );
      SetPage((prev) => prev + 1);
  ...
  return (
    <div className="w-screen h-screen relative">
      ...
      <div className="h-[90%] w-full">
        <LargeCards cardData={cardData} getData={getData} title="movie" />
      </div>
    </div>
  );
}
