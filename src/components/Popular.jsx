...
function Popular() {
  const [catagory1, setCatagory1] = useState("tv");
  const options1 = ["tv", "movie"];
  ...
  const getData = async () => {
    try {
      const { data } = await axios.get(
        `/${catagory1}/popular?language=en-US&page=${page}`
      );
      SetPage((prev) => prev + 1);
  ...
  return (
    <div className="w-screen h-screen relative">
      <div className="w-full h-[10%] flex justify-between item bg-zinc-800 sticky top-0">
        <div className="flex items-center gap-11 text-3xl">
          <i
            ...
            onClick={() => navigate(-1)}
          ></i>
          <h1 className="text-blue-400">Popular</h1>
        </div>
      ...
      <div className="h-[90%] w-full">
        <LargeCards cardData={cardData} getData={getData} />
      </div>
    </div>
  );
}
